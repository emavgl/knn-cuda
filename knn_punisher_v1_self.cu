...
__global__ void knnPunisher(float2* kOutput, int* d_classes, int numels, int newels, int k, int classes_num, int* again);
...
{  
  ...
  while (again != 0)
  {
      ...
      knnPunisher<<<numBlocks, blockSize, newels*sizeof(int)*2>>>(d_kOutput, d_classes, numels, newels, k, classes_num, d_again);
...
__global__ void knnPunisher(float2* kOutput, int* classes, int numels, int newels, int k, int classes_num, int* again)
{
